...
inline std::uint32_t le_read(std::ifstream& file)
{
	std::uint32_t value;
	file >> value;
	return value;
...
std::vector<std::uint32_t> le_read_block(std::ifstream& file, std::size_t n = 1)
{
...
CelHeader read_cell_header(std::string const& file_path)
{
	...
	std::vector<std::uint32_t> const frame_offsets = le_read_block(cel_file, cell_frames);
...
int main()
{
	CelHeader header = read_cell_header("D:/development/tmp/pngs2cel/cels/test_2cels.cel");
